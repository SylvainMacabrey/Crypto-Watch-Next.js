...
export default function Home({ res }) {
  return (
    <Layout page="Crypto Watch - Accueil" >
      <ul className="flex justify-around py-10 grid grid-cols-3 gap-4">
        { res.map((crypto, index) => (
          <li key={ index } className="relative hover:shadow-md p-8 border border-blue-300 bg-blue-100 md:w-auto flex-1 mx-5">
            <Link href={ `/${crypto.id}` }>
...
export async function getStaticProps(context) {
  try {
    const res = await fetch('https://api.nomics.com/v1/currencies/ticker?key=c6295acdaa827ddd4707a19e1705f57f&ids=BTC,ETH,LTC,ADA,XRP&interval=1d,30d,%20365d&convert=EUR')
                      .then(res => res.json())
    return {
      props: { res }
    }
